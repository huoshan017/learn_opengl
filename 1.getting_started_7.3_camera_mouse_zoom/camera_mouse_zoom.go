...
var (
	...
	yaw                  float64 = -90.0
	pitch                float64 = 0.0
	lastX                float64 = 800.0 / 2.0
	lastY                float64 = 600.0 / 2.0
	fov                  float64 = 45.0
...
func main() {
	...
	// -------------------------------------
	var shader = gl.NewShader("7.3.camera.vs", "7.3.camera.fs")
	...
	for !window.ShouldClose() {
		...
		projection := mgl32.Perspective(float32(fov)*math.Pi/180, SRC_WIDTH/SRC_HEIGHT, 0.1, 100.0)
...
func mouseCallback(_ *glfw.Window, xposIn, yposIn float64) {
	...
	var front = mgl32.Vec3{
		float32(math.Cos(float64(yaw)*math.Pi/360) * math.Cos(float64(pitch)*math.Pi/360)),
		float32(math.Sin(float64(pitch) * math.Pi / 360)),
		float32(math.Sin(float64(yaw)*math.Pi/360) * math.Cos(float64(pitch)*math.Pi/360)),
	}
