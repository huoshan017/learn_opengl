...
type DistanceWindowList struct {
	list []struct {
		distance float32
		window   mgl32.Vec3
	}
...
func (l *DistanceWindowList) Len() int {
	return len(l.list)
}

func (l *DistanceWindowList) Swap(i, j int) {
	l.list[i], l.list[j] = l.list[j], l.list[i]
}

func (l *DistanceWindowList) Less(i, j int) bool {
	return l.list[i].distance < l.list[j].distance
}
...
func main() {
	...
	// -------------------------------------
	var shader = gl.NewShader("3.2.blending.vs", "3.2.blending.fs")
	...
	gl.BufferData(gl.ARRAY_BUFFER, len(transparentVertices)*4, unsafe.Pointer(&transparentVertices[0]), gl.STATIC_DRAW)
	gl.VertexAttribPointer(0, 3, gl.FLOAT, false, 5*4, 0)
	gl.EnableVertexAttribArray(0)
	gl.VertexAttribPointer(1, 2, gl.FLOAT, false, 5*4, 3*4)
	gl.EnableVertexAttribArray(1)
	...
	for !window.ShouldClose() {
		...
		for i := 0; i < len(windows); i++ {
			distance := camera.Position().Sub(windows[i]).Len()
			distanceList[i] = struct {
				distance float32
				window   mgl32.Vec3
			}{
				distance: distance,
		...
		var distanceWindowList = DistanceWindowList{
			list: distanceList,
		}
		sort.Sort(&distanceWindowList)
		...
		for i := 0; i < len(distanceWindowList.list); i++ {
			a := &distanceWindowList.list[i].window
